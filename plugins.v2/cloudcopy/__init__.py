...
class CloudCopy(_PluginBase):
    # 插件名称
    plugin_name = "拷贝到云盘"
    # 插件描述
    plugin_desc = "监控目录文件变化，自动复制到云盘。"
    # 插件图标
    ...
    # 作者主页
    author_url = "https://github.com/wdmcheng"
    # 插件配置项ID前缀
    plugin_config_prefix = "cloud_copy_"
    # 加载顺序
    plugin_order = 4
    # 可使用的用户级别
    auth_level = 1
            ...
            for mon_path in monitor_dirs:
                ...
                if self._enabled:
                    ...
                    try:
                        ...
                        observer.start()
                        logger.info(f"{mon_path} 的拷贝实时监控服务启动")
                    except Exception as e:
                        err_msg = str(e)
                        if "inotify" in err_msg and "reached" in err_msg:
                            logger.warn(
                                f"拷贝实时监控服务启动出现异常：{err_msg}，请在宿主机上（不是docker容器内）执行以下命令并重启："
                                + """
                            ...
                            logger.error(f"{mon_path} 启动拷贝实时监控失败：{err_msg}")
                        self.systemmessage.put(f"{mon_path} 启动拷贝实时监控失败：{err_msg}")

            # 运行一次定时服务
            if self._onlyonce:
                logger.info("拷贝实时监控服务启动，立即运行一次")
                self._scheduler.add_job(name="拷贝实时监控",
    ...
    def remote_sync(self, event: Event):
        ...
        if event:
            self.post_message(channel=event.event_data.get("channel"),
                              title="拷贝实时监控目录同步完成！", userid=event.event_data.get("user"))
        ...
        try:
            ...
            with lock:
                ...
                if self._keep_structure:
                    target_dir.renaming = False
                    target_file = Path(os.path.join(target_dir.library_path, os.path.relpath(os.path.abspath(file_path), os.path.abspath(mon_path))))
